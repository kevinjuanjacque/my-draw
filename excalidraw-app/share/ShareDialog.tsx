...
export type ShareDialogProps = {
  ...
  handleClose: () => void;
  onCreateBoardReadShare: () => Promise<string>;
    ...
    <>
      ...
      {props.type === "share" && (
        <>
          ...
          <div className="ShareDialog__picker__description">
            This link loads a snapshot of the current board. It is separate from
            ...
            {boardLink ? (
              <>
                ...
                <FilledButton
                  ...
                  onClick={async () => {
                    try {
                      await copyTextToSystemClipboard(boardLink);
                      onCopy();
                    } catch (error: unknown) {
                      setErrorMessage(
                        error instanceof Error
                          ? error.message
                          : "Could not copy the board link.",
                      );
                    }
                  }}
                />
              ...
              <FilledButton
                size="large"
                label="Create read-only link"
                icon={LinkIcon}
                onClick={async () => {
                  setErrorMessage(null);
                  try {
                    setBoardLink(await props.onCreateBoardReadShare());
                  } catch (error: unknown) {
                    setErrorMessage(
                      error instanceof Error
                        ? error.message
                        : "Could not create a board link.",
                    );
                  }
                }}
              />
...
export const ShareDialog = (props: {
  ...
  return (
    <ShareDialogInner
      ...
      collabAPI={props.collabAPI}
      onCreateBoardReadShare={props.onCreateBoardReadShare}
